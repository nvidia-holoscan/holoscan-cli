...
def test_setup_cmake_dryrun_refreshes_new_repository(monkeypatch):
    package_calls = []
    commands = []
    writes = []
    monkeypatch.setattr(host_setup, "_apt_updated", False)
    monkeypatch.setattr(host_setup, "get_installed_package_version", lambda _: None)
    monkeypatch.setattr(host_setup, "get_ubuntu_codename", lambda: "noble")
    monkeypatch.setattr(
        host_setup,
        "install_packages_if_missing",
        lambda packages, dry_run=False, **kwargs: package_calls.append((packages, dry_run)),
    )
    monkeypatch.setattr(
        host_setup,
        "write_system_file",
        lambda path, content, dry_run=False: writes.append((path, content, dry_run)),
    )
    monkeypatch.setattr(
        host_setup,
        "run_command",
    ...
    host_setup.setup_cmake(dry_run=True)

    assert package_calls == [
        (["gpg", "wget"], True),
        (["cmake", "cmake-curses-gui"], True),
    ]
    assert writes == [
        (
    ...
    assert commands == [(["apt-get", "update"], {"dry_run": True, "as_root": True})]
    assert host_setup._apt_updated is True


def test_setup_sccache_uses_bundled_script_and_requested_version(monkeypatch, tmp_path):
    from holoscan_cli.utils import holohub as holohub_utils

    script = tmp_path / "sccache.sh"
    script.write_text("#!/usr/bin/env bash\n", encoding="utf-8")
    commands = []
    monkeypatch.setattr(holohub_utils, "get_holohub_setup_scripts_dir", lambda: tmp_path)
    monkeypatch.setattr(
        host_setup,
        "run_command",
        lambda cmd, **kwargs: commands.append((cmd, kwargs)),
    ...
    host_setup.setup_sccache(min_version="1.2.3-rapids.4", dry_run=True)

    assert commands[0][0] == ["bash", str(script)]
    assert commands[0][1]["dry_run"] is True
    assert commands[0][1]["env"]["SCCACHE_MIN_VERSION"] == "1.2.3-rapids.4"
...
def test_setup_ngc_cli_uses_arm64_archive_on_aarch64(monkeypatch, tmp_path):
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setattr(host_setup.os, "geteuid", lambda: 1000)
    monkeypatch.setattr(host_setup.shutil, "which", lambda _: None)
    monkeypatch.setattr(host_setup.platform, "machine", lambda: "aarch64")
