...
def make_local_build_command(
    ...
    # ambient CMAKE_BUILD_TYPE disappears at the container boundary.
    command.extend(
        [
            "--build-type",
...
def register_build_parser(
    ...
    add_local_container_args(parser, "Build")
...
def handle_build(cli, args: argparse.Namespace) -> None:
    ...
    if is_local_mode:
        ...
        # ``container.build()`` normally applies --cuda. Apply image-affecting
        # overrides before the skip branch so --no-docker-build keeps the same
        # image selection.
...
def build_project_locally(
    ...
    sdk_dir = resolve_local_sdk_dir(cli.DEFAULT_SDK_DIR, local_sdk_root, environ=build_env)
